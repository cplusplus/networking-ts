...
\pnum
The class \tcode{basic_socket_streambuf<Protocol, Clock, WaitTraits>} associates both the input sequence and the output sequence with a socket. The input and output sequences do not support seeking. \enternote The input and output sequences are independent as a stream socket provides full duplex I/O. \exitnote
...
\pnum
Instances of class template \tcode{basic_socket_streambuf} meet the requirements of \tcode{Destructible} (C++Std [destructible]), \tcode{MoveConstructible} (C++Std [moveconstructible]), and \tcode{MoveAssignable} (C++Std [moveassignable]).
...
\pnum
\postconditions \tcode{expiry() == clock_type::max()}.
\end{itemdescr}
...
\item
\tcode{rhs_a.expiry() == clock_type::max()}
\item
...
\begin{itemdescr}
\pnum
\effects If a put area exists, calls \tcode{overflow(traits_type::eof())} to flush characters. \enternote The socket is closed by the \tcode{basic_stream_socket<protocol_type>} destructor. \exitnote
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Behaves according to the description of \tcode{basic_streambuf<char>::overflow(c)}, except that the behavior of "consuming characters" is performed by output of the characters to the socket as if by one or more calls to POSIX sendmsg, and \tcode{ec_} is set to reflect the error code produced by the operation. If the operation does not complete before the absolute timeout specified by \tcode{expiry_}, the socket is closed and \tcode{ec_} is set to \tcode{errc::timed_out}.
...
\pnum
Instances of class template \tcode{basic_socket_iostream} meet the requirements of \tcode{Destructible} (C++Std [destructible]), \tcode{MoveConstructible} (C++Std [moveconstructible]), and \tcode{MoveAssignable} (C++Std [moveassignable]).
...
\begin{itemdescr}
\pnum
\returns \tcode{const_cast<basic_socket_streambuf<protocol_type, clock_type, traits_type>*>(std::addressof(sb_))}.
\end{itemdescr}
