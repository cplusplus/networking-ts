...
namespace std::experimental::net::inline @\namespacever@ {
  ...
  private:
    basic_stream_socket<protocol_type, executor_type> socket_; // \expos
...
\begin{itemdescr}
\pnum
\effects If a put area exists, calls \tcode{overflow(traits_type::eof())} to flush characters. \begin{note} The socket is closed by the \tcode{basic_stream_socket<protocol_type, executor_type>} destructor. \end{note}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects If a put area exists, calls \tcode{overflow(traits_type::eof())} to flush characters. Regardless of whether the preceding call fails or throws an exception, the function closes the socket as if by \tcode{basic_socket<protocol_type, executor_type>::close(ec_)}. If any of the calls made by the function fail, \tcode{close} fails by returning a null pointer. If one of these calls throws an exception, the exception is caught and rethrown after closing the socket.
...
namespace std::experimental::net::inline @\namespacever@ {
  ...
  private:
    basic_socket_streambuf<protocol_type, clock_type, wait_traits_type, executor_type> sb_; // \expos
  };
...
\begin{itemdescr}
\pnum
Let \tcode{SB} be the type \tcode{basic_socket_streambuf<protocol_type, clock_type, wait_traits_type, executor_type>}.
