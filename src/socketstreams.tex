...
\ednote{Using \tcode{addressof} here resolves LWG issue 344x.}

\begin{itemdescr}
\pnum
\effects Initializes the base class as \tcode{basic_iostream<char>(\changed{\&sb_}{address(sb_)}))}, value-initializes \tcode{sb_}, and performs \tcode{setf(std::ios_base::unitbuf)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Initializes the base class as \tcode{basic_iostream<char>(\changed{\&sb_}{address(sb_)}))}, initializes \tcode{sb_} with \tcode{std::move(s)}, and performs \tcode{setf(std::ios_base::unitbuf)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Move constructs from the rvalue \tcode{rhs}. This is accomplished by move constructing the base class, and the contained \tcode{basic_socket_streambuf}. Next \tcode{basic_iostream<char>::set_rdbuf(\changed{\&sb_}{address(sb_)}))} is called to install the contained \tcode{basic_socket_streambuf}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Initializes the base class as \tcode{basic_iostream<char>(\changed{\&sb_}{address(sb_)}))}, value-initializes \tcode{sb_}, and performs \tcode{setf(std::ios_base::unitbuf)}. Then calls \tcode{rdbuf()->connect(forward<Args>(args)...)}. If that function returns a null pointer, calls \tcode{setstate(failbit)}.
\end{itemdescr}
