...
\pnum
For a type \tcode{Clock} meeting the \tmpoldconcept{Clock} requirements~(\CppXref{time.clock.req}), a type \tcode{X} meets the \tmpnewoldconcept{WaitTraits} requirements if it \changed{satisfies}{meets} the requirements listed below.

\pnum
In Table~\ref{tab:timer.reqmts.waittraits.requirements},
\tcode{t} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{Clock::time_point};
and \tcode{d} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{Clock::duration}.

\indextext{requirements!\idxnewoldconcept{WaitTraits}}%
\begin{libreqtab3}
{\tmpnewoldconcept{WaitTraits} requirements}
...
\pnum
Class template \tcode{wait_traits} \changed{satisfies}{meets} the \tmpnewoldconcept{WaitTraits}~(\ref{timer.reqmts.waittraits}) type requirements. Template argument \tcode{Clock} is a type meeting the \tmpoldconcept{Clock} requirements (\CppXref{time.clock.req}).
...
\pnum
\changed{Instances}{Specializations} of class template \tcode{basic_waitable_timer} meet the requirements of \tmpoldconcept{Destructible}~(\CppXref{destructible}), \tmpoldconcept{MoveConstructible}~(\CppXref{moveconstructible}), and \tmpoldconcept{MoveAssignable}~(\CppXref{moveassignable}).
