...
namespace ip {
  ...
  constexpr strong_ordering operator<=>(const address\&, const address\&) noexcept;
  ...
  constexpr strong_ordering operator<=>(const address_v4\&, const address_v4\&) noexcept;
  ...
  constexpr strong_ordering operator<=>(const address_v6\&, const address_v6\&) noexcept;
  ...
  template<class InternetProtocol>
    strong_ordering operator<=>(const basic_endpoint<InternetProtocol>\&,
                                const basic_endpoint<InternetProtocol>\&);
  ...
  template<class InternetProtocol@\addedcode{, class Executor = executor}@>
...
\pnum
A type \tcode{X} meets the \defnnewoldconcept{InternetProtocol} requirements if it meets the requirements of \newoldconcept{AcceptableProtocol} (\ref{socket.reqmts.acceptableprotocol}), as well as the additional requirements listed below.

\pnum
In the table below,
\tcode{a} and \tcode{b} denote  values of type (possibly const) \tcode{X}.

\begin{libreqtab3}
{\newoldconcept{InternetProtocol} requirements}
...
\pnum
A type \tcode{X} meets the \defnnewoldconcept{MulticastGroupSocketOption} requirements if it meets the requirements of \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), \oldconcept{CopyAssignable} (\CppXref{copyassignable}), and \newoldconcept{SettableSocketOption}~(\ref{socket.reqmts.settablesocketoption}), as well as the additional requirements listed below.

\pnum
In the table below,
\tcode{a} denotes a  value of type (possibly const) \tcode{address},
\tcode{b} and \tcode{c} denote (possibly const) values of type \tcode{address_v4},
\tcode{d} denotes a  value of type (possibly const) \tcode{address_v6},
\tcode{e} denotes a  value of type (possibly const) \tcode{unsigned int},
and \tcode{u} denotes an identifier.

\begin{libreqtab3}
{\newoldconcept{MulticastGroupSocketOption} requirements}
...
\pnum
In this document, types that meet the \newoldconcept{MulticastGroupSocketOption} requirements are defined as follows.
...
\indexlibrary{\idxcode{address}}%
\pnum
Class \tcode{address} meets the requirements for \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \oldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\pnum
\tcode{address_v4} meets the requirements for \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \oldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\begin{itemdescr}
\pnum
\throws \tcode{out_of_range} if any element of \tcode{bytes} is not in the range \tcode{[0, 0xFF]}. \begin{note} For implementations where \tcode{numeric_limits<unsigned char>::max() == 0xFF}, no out-of-range detection is needed. \end{note}
...
\begin{itemdescr}
\pnum
\throws \tcode{out_of_range} if \tcode{val} is not in the range \tcode{[0, 0xFFFFFFFF]}. \begin{note} For implementations where \tcode{numeric_limits<address_v4::uint_type>::max() == 0xFFFFFFFF}, no out-of-range detection is needed. \end{note}
...
\pnum
\throws \tcode{bad_address_cast} if \tcode{a.is_v4_mapped()} is \tcode{false}.
\end{itemdescr}
...
\pnum
\tcode{address_v6} meets the requirements for \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \oldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\begin{itemdescr}
\pnum
\throws \tcode{out_of_range} if any element of \tcode{bytes} is not in the range \crange{0}{0xFF}. \begin{note} For implementations where \tcode{numeric_limits<unsigned char>::max() == 0xFF}, no out-of-range detection is needed. \end{note}
...
\begin{itemdescr}
\pnum
The template specializations are enabled (\CppXref{unord.hash}).
\end{itemdescr}
...
\pnum
Specializations of \tcode{basic_address_iterator} meet the requirements for input iterators (\CppXref{input.iterators}).
...
\pnum
Specializations of \tcode{basic_address_range} meet the requirements for \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \oldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\pnum
\tcode{network_v4} meets the requirements for \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \oldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\pnum
\throws \tcode{out_of_range} if \tcode{prefix_len < 0} or \tcode{prefix_len > 32}.
\end{itemdescr}
...
\pnum
\throws \tcode{invalid_argument} if \tcode{mask} contains non-contiguous non-zero bits, or if the most significant bit is zero and any other bits are non-zero.
\end{itemdescr}
...
\pnum
\tcode{network_v6} meets the requirements for \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \oldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\pnum
\throws \tcode{out_of_range} if \tcode{prefix_len < 0} or \tcode{prefix_len > 128}.
\end{itemdescr}
...
\pnum
Specializations of the \tcode{basic_endpoint} class template meet the requirements for an \newoldconcept{Endpoint}~(\ref{socket.reqmts.endpoint}).
...
\begin{itemdescr}
\pnum
\expects \tcode{proto == protocol_type::v4() || proto == protocol_type::v6()}.
...
\begin{itemdescr}
\pnum
\throws \tcode{length_error} if either of the following conditions is \tcode{true}:
\begin{itemize}
...
\begin{itemdescr}
\pnum
\mandates \tcode{is_same_v<allocator_traits<Allocator>::value_type, char>} is \tcode{true}.
...
\pnum
The class template \tcode{basic_resolver_results} meets the requirements of a sequence container (\CppXref{sequence.reqmts}), except that only the operations defined for const-qualified sequence containers are supported. The class template \tcode{basic_resolver_results} supports forward iterators.
...
namespace ip {
  ...
  public:
    ...
    @\addedcode{  basic_resolver<InternetProtocol, OtherExecutor>;}@

    // \ref{internet.resolver.cons}, construct / copy / destroy

    explicit basic_resolver(@\removedcode{io_context\& ctx}\addedcode{const executor_type\& ex}@);
    @\addedcode{template<class ExecutionContext> explicit basic_resolver(ExecutionContext\& ctx);}@
...
\begin{itemdescr}
\addedpnum
\added{\postconditions \tcode{get_executor() == ctx.get_executor()}.}
...
\begin{itemdescr}
\pnum
\mandates For the overloads with an \tcode{Allocator}, \tcode{is_same_v<allocator_traits<Allocator>::value_type, char>} is \tcode{true}.
...
namespace ip {
  ...
  public:
    ...
    @\addedcode{template<class Executor>}@
    @\addedcode{  using socket_for = basic_stream_socket<tcp, Executor>}@
...
\pnum
The \tcode{tcp} class meets the requirements for an \newoldconcept{InternetProtocol}~(\ref{internet.reqmts.protocol}).
...
namespace ip {
  ...
  public:
    ...
    @\addedcode{template<class Executor>}@
    @\addedcode{  using socket_for = basic_datagram_socket<udp, Executor>}@
...
\pnum
The \tcode{udp} class meets the requirements for an \newoldconcept{InternetProtocol}~(\ref{internet.reqmts.protocol}).
...
Meets the \newoldconcept{MulticastGroupSocketOption} (\ref{internet.reqmts.opt.mcast}) type requirements. Requests that the socket join the specified multicast group.  \\ \rowsep
...
\tcode{ipv6_mreq} if \tcode{F == AF_INET6}, otherwise \tcode{ip_mreq}  &
Meets the \newoldconcept{MulticastGroupSocketOption} (\ref{internet.reqmts.opt.mcast}) type requirements. Requests that the socket leave the specified multicast group.  \\ \rowsep
...
\tcode{int}  &
Meets the \newoldconcept{BooleanSocketOption} (\ref{socket.reqmts.opt.bool}) type requirements. Determines whether multicast datagrams are delivered back to the local application.  \\
...
\pnum
\tcode{outbound_interface} meets the requirements for \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), \oldconcept{CopyAssignable} (\CppXref{copyassignable}), and \newoldconcept{SettableSocketOption}~(\ref{socket.reqmts.settablesocketoption}).
