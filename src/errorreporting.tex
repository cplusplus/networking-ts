...
\pnum
\enterexample The POSIX specification for \tcode{shutdown} lists \tcode{EBADF} as one of its possible errors. If a function that is specified ``as if'' implemented by \tcode{shutdown} fails with \tcode{EBADF} then the following condition holds for the \tcode{error_code} value \tcode{ec}: \tcode{ec == errc::bad_file_descriptor} \exitexample
