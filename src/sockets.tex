...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{Endpoint} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \tmpoldconcept{CopyAssignable} (\CppXref{copyassignable}), as well as the additional requirements listed below.
...
\begin{libreqtab3}
{\tmpnewoldconcept{Endpoint} requirements}
...
type meeting \tmpnewoldconcept{Protocol}~(\ref{socket.reqmts.protocol}) requirements  &
  \\ \rowsep
...
and \tcode{s} denotes a \removed{(possibly const)} value \removed{of a type}
that is convertible to \tcode{size_t} and denotes a size in bytes.

\begin{libreqtab3}
{\tmpnewoldconcept{Endpoint} requirements for extensible implementations}
...
\tcode{a.data()}  &
\tcode{const void*}  &
Returns a pointer suitable for passing as the \tcode{address} argument to functions such as POSIX \tcode{connect}, or as the \tcode{dest_addr} argument to functions such as POSIX \tcode{sendto}. The implementation \changed{shall perform}{performs} a \tcode{static_cast} on the pointer to convert it to \tcode{const sockaddr*}.  \\ \rowsep

\tcode{b.data()}  &
\tcode{void*}  &
Returns a pointer suitable for passing as the \tcode{address} argument to functions such as POSIX \tcode{accept}, \tcode{getpeername}, \tcode{getsockname} and \tcode{recvfrom}. The implementation \changed{shall perform}{performs} a \tcode{static_cast} on the pointer to convert it to \tcode{sockaddr*}.  \\ \rowsep
...
\tcode{b.resize(s)}  &
  &
\changed{pre:}{\expects} \tcode{s >= 0}\br
\changed{post:}{\postconditions} \tcode{a.size() == s}\br
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{EndpointSequence} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}) and \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), as well as the additional requirements listed below.

\pnum
In the table below, \tcode{x} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{X}.

\begin{libreqtab3}
{\tmpnewoldconcept{EndpointSequence} requirements}
...
A type meeting the requirements for forward iterators (\CppXref{forward.iterators}) whose value type is convertible to a type \changed{satisfying}{meeting} the \tmpnewoldconcept{Endpoint}~(\ref{socket.reqmts.endpoint}) requirements.  &
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{Protocol} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), and \tmpoldconcept{CopyAssignable} (\CppXref{copyassignable}), as well as the additional requirements listed below.

\begin{libreqtab3}
{\tmpnewoldconcept{Protocol} requirements}
...
\begin{libreqtab3}
{\tmpnewoldconcept{Protocol} requirements for extensible implementations}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{AcceptableProtocol} requirements if it \changed{satisfies}{meets} the requirements of \tmpnewoldconcept{Protocol}~(\ref{socket.reqmts.protocol}) as well as the additional requirements listed below.

\begin{libreqtab3}
{\tmpnewoldconcept{AcceptableProtocol} requirements}
...
\tcode{X::socket}  &
A type that \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}) and \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}), and that is publicly and unambiguously derived from \tcode{basic_socket<X>}.  &
  \\
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{GettableSocketOption} requirements if it \changed{satisfies}{meets} the requirements listed below.

\pnum
In the table below, \tcode{a} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{X},
\tcode{b} denotes a value of type \tcode{X},
\tcode{p} denotes a value of a (possibly const) type
that meets the \tmpnewoldconcept{Protocol}~(\ref{socket.reqmts.protocol}) requirements,
and \tcode{s} denotes a value of a (possibly const) type
that is convertible to \tcode{size_t} and denotes a size in bytes.

\begin{libreqtab3}
{\tmpnewoldconcept{GettableSocketOption} requirements for extensible implementations}
...
\tcode{b.resize(p,s)}  &
  &
\changed{post:}{\postconditions} \tcode{b.size(p) == s}. Passed the value contained in the \tcode{option_len} argument to POSIX \tcode{getsockopt} (or equivalent) after successful completion of the function. Permitted to throw an exception if the socket option object \tcode{b} does not support the specified size.  \\
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{SettableSocketOption} requirements if it \changed{satisfies}{meets} the requirements listed below.

\pnum
In the table below, \tcode{a} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{X},
\tcode{p} denotes a \removed{(possibly const)} value of a {(possibly const)} type
that meets the \tcode{Protocol}~(\ref{socket.reqmts.protocol}) requirements,
and \tcode{u} denotes an identifier.

\begin{libreqtab3}
{\tmpnewoldconcept{SettableSocketOption} requirements for extensible implementations}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{BooleanSocketOption} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), \tmpoldconcept{CopyAssignable} (\CppXref{copyassignable}), \tmpnewoldconcept{GettableSocketOption}~(\ref{socket.reqmts.gettablesocketoption}), and \tmpnewoldconcept{SettableSocketOption}~(\ref{socket.reqmts.settablesocketoption}), \tcode{X} is contextually convertible to bool, and \tcode{X} \changed{satisfies}{meets} the additional requirements listed below.

\pnum
In the table below, \tcode{a} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{X},
\tcode{v} denotes a \removed{(possibly const)} value of type {(possibly const)} \tcode{bool},
and \tcode{u} denotes an identifier.

\begin{libreqtab3}
{\tmpnewoldconcept{BooleanSocketOption} requirements}
...
\tcode{X u;}  &
  &
\changed{post:}{\postconditions} \tcode{!u.value()}.  \\ \rowsep

\tcode{X u(v);}  &
  &
\changed{post:}{\postconditions} \tcode{u.value() == v}.  \\ \rowsep
...
\pnum
In this document, types that \changed{satisfy}{meet} the \tmpnewoldconcept{BooleanSocketOption} requirements are defined as follows.
...
\begin{itemdescr}
\pnum
\changed{\remarks}{\throws} \tcode{length_error} if \tcode{s} is not a valid data size for the protocol specified by \tcode{p}.
\end{itemdescr}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{IntegerSocketOption} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), \tmpoldconcept{CopyAssignable} (\CppXref{copyassignable}), \tmpnewoldconcept{GettableSocketOption}~(\ref{socket.reqmts.gettablesocketoption}), and \tmpnewoldconcept{SettableSocketOption}~(\ref{socket.reqmts.settablesocketoption}), as well as the additional requirements listed below.

\pnum
In the table below, \tcode{a} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{X},
\tcode{v} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{int},
and \tcode{u} denotes an identifier.

\begin{libreqtab3}
{\tmpnewoldconcept{IntegerSocketOption} requirements}
...
\tcode{X u;}  &
  &
\changed{post:}{\postconditions} \tcode{u.value() == 0}.  \\ \rowsep
...
\pnum
In this document, types that \changed{satisfy}{meet} the \tmpnewoldconcept{IntegerSocketOption} requirements are defined as follows.
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{IoControlCommand} requirements if it \changed{satisfies}{meets} the requirements listed below.
...
\begin{libreqtab3}
{\tmpnewoldconcept{IoControlCommand} requirements for extensible implementations}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{ConnectCondition} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}) and \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), as well as the additional requirements listed below.

\pnum
In the table below, \tcode{x} denotes a value of type \tcode{X},
\tcode{ec} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{error_code},
and \tcode{ep} denotes a \removed{(possibly const)} value of a \added{(possibly const)} type \changed{satisfying}{meeting}
the endpoint~(\ref{socket.reqmts.endpoint}) requirements.

\begin{libreqtab3}
{\tmpnewoldconcept{ConnectCondition} requirements}
...
\tcode{SO_BROADCAST}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{BooleanSocketOption}~(\ref{socket.reqmts.opt.bool}) type requirements. Determines whether a socket permits sending of broadcast messages, if supported by the protocol.  \\ \rowsep
...
\tcode{SO_DEBUG}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{BooleanSocketOption}~(\ref{socket.reqmts.opt.bool}) type requirements. Determines whether debugging information is recorded by the underlying protocol.  \\ \rowsep
...
\tcode{SO_DONTROUTE}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{BooleanSocketOption}~(\ref{socket.reqmts.opt.bool}) type requirements. Determines whether outgoing messages bypass standard routing facilities.  \\ \rowsep
...
\tcode{SO_KEEPALIVE}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{BooleanSocketOption}~(\ref{socket.reqmts.opt.bool}) type requirements. Determines whether a socket permits sending of keep_alive messages, if supported by the protocol.  \\ \rowsep
...
\tcode{SO_OOBINLINE}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{BooleanSocketOption}~(\ref{socket.reqmts.opt.bool}) type requirements. Determines whether out-of-band data (also known as urgent data) is received inline.  \\ \rowsep
...
\tcode{SO_RCVBUF}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{IntegerSocketOption}~(\ref{socket.reqmts.opt.int}) type requirements. Specifies the size of the receive buffer associated with a socket.  \\ \rowsep
...
\tcode{SO_RCVLOWAT}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{IntegerSocketOption}~(\ref{socket.reqmts.opt.int}) type requirements. Specifies the minimum number of bytes to process for socket input operations.  \\ \rowsep
...
\tcode{SO_REUSEADDR}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{BooleanSocketOption}~(\ref{socket.reqmts.opt.bool}) type requirements. Determines whether the validation of endpoints used for binding a socket should allow the reuse of local endpoints, if supported by the protocol.  \\ \rowsep
...
\tcode{SO_SNDBUF}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{IntegerSocketOption}~(\ref{socket.reqmts.opt.int}) type requirements. Specifies the size of the send buffer associated with a socket.  \\ \rowsep
...
\tcode{SO_SNDLOWAT}  &
\tcode{int}  &
\changed{Satisfies}{Meets} the \tmpnewoldconcept{IntegerSocketOption}~(\ref{socket.reqmts.opt.int}) type requirements. Specifies the minimum number of bytes to process for socket output operations.  \\
...
\pnum
\changed{Instances}{Specializations} of class template \tcode{basic_socket} meet the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}), and \tmpoldconcept{MoveAssignable} (\CppXref{moveassignable}).
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{native_socket} is a native handle to an open socket.
...
\pnum
\changed{\remarks This constructor shall not participate in overload resolution unless
\tcode{OtherProtocol} is implicitly convertible to \tcode{Protocol}.}
{%
...
\pnum
\changed{\remarks This assignment operator shall not participate in overload resolution unless
\tcode{OtherProtocol} is implicitly convertible to \tcode{Protocol}.}
{%
\constraints \tcode{is_convertible_v<OtherProtocol, Protocol>} is \tcode{true}.
\ednote{should we require assignable not convertible?}
}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{is_open() == true}.
...
\pnum
\changed{Instances}{Specializations} of class template \tcode{basic_datagram_socket} meet the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}), and \tmpoldconcept{MoveAssignable} (\CppXref{moveassignable}).
...
\pnum
\changed{Instances}{Specializations} of class template \tcode{basic_stream_socket} meet the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}), \tmpoldconcept{MoveAssignable} (\CppXref{moveassignable}), \tmpnewoldconcept{SyncReadStream}~(\ref{buffer.stream.reqmts.syncreadstream}), \tmpnewoldconcept{SyncWriteStream}~(\ref{buffer.stream.reqmts.syncwritestream}), \tmpnewoldconcept{AsyncReadStream}~(\ref{buffer.stream.reqmts.asyncreadstream}), and \tmpnewoldconcept{AsyncWriteStream}~(\ref{buffer.stream.reqmts.asyncwritestream}).
...
\pnum
\changed{Instances}{Specializations} of class template \tcode{basic_socket_acceptor} meet the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}), and \tmpoldconcept{MoveAssignable} (\CppXref{moveassignable}).
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{native_acceptor} is a native handle to an open acceptor.
