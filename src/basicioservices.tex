...
\begin{itemdescr}
\pnum
\effects If \tcode{running_in_this_thread()} is \tcode{true}, calls DECAY_COPY\tcode{(forward<Func>(f))()} (C++Std [thread.decaycopy]). \enternote If \tcode{f} exits via an exception, the exception propagates to the caller of \tcode{dispatch()}. \exitnote Otherwise, calls \tcode{post(forward<Func>(f), a)}.
\end{itemdescr}
