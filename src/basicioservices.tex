...
\pnum
The class \tcode{io_context} \changed{satisfies}{meets} the \changed{ExecutionContext}{\newoldconcept{ExecutionContext}} type requirements~(\ref{async.reqmts.executioncontext}).
...
@\removed{inline }@namespace @\added{std::experimental::net::inline}@ @\namespacever@ {
  ...
  @\removed{bool operator!=(const io_context::executor_type\& a,
                  const io_context::executor_type\& b) noexcept;}@
...
\pnum
\tcode{io_context::executor_type} is a type \changed{satisfying}{meeting} the \changed{Executor}{\newoldconcept{Executor}} requirements~(\ref{async.reqmts.executor}). Objects of type \tcode{io_context::executor_type} are associated with an \tcode{io_context}, and function objects submitted using the \tcode{dispatch}, \tcode{post}, or \tcode{defer} member functions will be executed by the \tcode{io_context} from within a run function.
...
\begin{itemdescr}
\pnum
\effects If \tcode{running_in_this_thread()} is \tcode{true}, calls \exposid{decay-copy}\tcode{(forward<Func>(f))()} (\CppXref{expos.only.func}).
\begin{note}
If \tcode{f} exits via an exception, the exception propagates to the caller of \tcode{dispatch()}.
\end{note}
Otherwise, calls \tcode{post(forward<Func>(f), a)}.
\end{itemdescr}
