...
\pnum
A \defn{dynamic buffer} encapsulates memory storage that may be automatically resized as required. Direct access to the elements is provided to permit them to be efficiently used with I/O operations. \begin{note} Such as the \tcode{send} or \tcode{receive} operations of a socket. \end{note}
...
\tcode{pos} denotes a (possibly const) value of type \tcode{size_t},
...
type meeting \newoldconcept{ConstBufferSequence}~(\ref{buffer.reqmts.constbuffersequence}) requirements.  &
 This type represents the underlying memory as non-modifiable bytes.  \\ \rowsep

\tcode{X::mutable_buffers_type}  &
type meeting \newoldconcept{MutableBufferSequence}~(\ref{buffer.reqmts.constbuffersequence}) requirements.  &
 This type represents the underlying memory as modifiable bytes.  \\ \rowsep

\tcode{x1.size()}  &
\tcode{size_t}  &
 Returns the number of bytes.  \\ \rowsep

\tcode{x1.max_size()}  &
\tcode{size_t}  &
Returns the maximum number of bytes that can be held by \tcode{x1}.  \\ \rowsep

\tcode{x1.capacity()}  &
\tcode{size_t}  &
Returns the maximum number of bytes that can be held by \tcode{x1} without requiring reallocation.  \\ \rowsep

\tcode{x1.data(pos, n)}  &
\tcode{X::const_buffers_type}  &
Returns a constant buffer sequence \tcode{u} that represents the region of underlying memory at offset \tcode{pos} and length \tcode{n}.  \\ \rowsep

\tcode{x.data(pos, n)}  &
\tcode{X::mutable_buffers_type}  &
Returns a mutable buffer sequence \tcode{u} that represents the region of underlying memory at offset \tcode{pos} and length \tcode{n}.  \\ \rowsep

\tcode{x.grow(n)}  &
  &
Adds \tcode{n} bytes of space at the end of the underlying memory. All constant or mutable buffer sequences previously obtained using \tcode{data()} are invalidated.  \\ \rowsep

\tcode{x.shrink(n)}  &
  &
Removes \tcode{n} bytes of space from the end of the underlying memory. If \tcode{n} is greater than the number of bytes, all bytes are discarded. All constant or mutable buffer sequences previously obtained using \tcode{data()} are invalidated.  \\ \rowsep

\tcode{x.consume(n)}  &
  &
Removes \tcode{n} bytes from beginning of the bytes. If \tcode{n} is greater than the number of bytes, all bytes are removed. All constant or mutable buffer sequences previously obtained using \tcode{data()} are invalidated.  \\
...
\pnum
\effects Initializes \tcode{vec_} with \tcode{vec} and \tcode{max_size_} with \tcode{maximum_size}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{min(vec_.size(), max_size_)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{min(vec_.capacity(), max_size_)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{buffer(buffer(vec_, max_size_) + pos, n)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Performs \tcode{vec_.resize(size() + n);}.

\pnum
\throws \tcode{length_error} if \tcode{size() > max_size() || max_size() - size() < n}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Performs \tcode{vec_.resize(n > size() ? 0 : size() - n);}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Initializes \tcode{str_} with \tcode{str} and \tcode{max_size_} with \tcode{str.max_size()}.
\end{itemdescr}
...
\pnum
\effects Initializes \tcode{str_} with \tcode{str} and \tcode{max_size_} with \tcode{maximum_size}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{min(str_.size(), max_size_)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{min(str_.capacity(), max_size_)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\returns \tcode{buffer(buffer(str_, max_size_) + pos, n)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Performs \tcode{str_.resize(size() + n);}.
...
\begin{itemdescr}
\pnum
\effects Performs \tcode{str_.resize(n > size() ? 0 : size() - n);}.
\end{itemdescr}
