...
@\removed{inline }@namespace @\added{std::experimental::net::inline}@ @\namespacever@ {
  ...
  template<class T>
    @\added{inline}@ constexpr bool is_mutable_buffer_sequence_v = is_mutable_buffer_sequence<T>::value;
  template<class T>
    @\added{inline}@ constexpr bool is_const_buffer_sequence_v = is_const_buffer_sequence<T>::value;
  template<class T>
    @\added{inline}@ constexpr bool is_dynamic_buffer_v = is_dynamic_buffer<T>::value;
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{MutableBufferSequence} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible}~(\CppXref{destructible}) and \tmpoldconcept{CopyConstructible}~(\CppXref{copyconstructible}), as well as the additional requirements listed in Table~\ref{tab:buffer.reqmts.mutablebuffersequence.requirements}.

\pnum
In Table~\ref{tab:buffer.reqmts.mutablebuffersequence.requirements},
\tcode{x} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{X},
and \tcode{u} denotes an identifier.

\indextext{requirements!\idxnewoldconcept{MutableBufferSequence}}%
\begin{LongTable}
{\tmpnewoldconcept{MutableBufferSequence} requirements}
...
and which \changed{satisfies}{meets} all the requirements for bidirectional iterators (\CppXref{bidirectional.iterators})
except that:
\begin{itemize}
...
\tcode{X u(x);}
  &
  &
\changed{post:}{\postconditions} \begin{codeblock}
equal(
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{ConstBufferSequence} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible}~(\CppXref{destructible}) and \tmpoldconcept{CopyConstructible}~(\CppXref{copyconstructible}), as well as the additional requirements listed in Table~\ref{tab:buffer.reqmts.constbuffersequence.requirements}.
...
\begin{LongTable}
{\tmpnewoldconcept{ConstBufferSequence} requirements}
...
The context in which these calls are evaluated \changed{shall ensure}{ensures} that
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{DynamicBuffer} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible}~(\CppXref{destructible}) and \tmpoldconcept{MoveConstructible}~(\CppXref{moveconstructible}), as well as the additional requirements listed in Table~\ref{tab:buffer.reqmts.dynamicbuffer.requirements}.
...
\begin{libreqtab3}
{\tmpnewoldconcept{DynamicBuffer} requirements}
...
\pnum
A \defn{read operation} is an operation that reads data into a mutable buffer sequence argument of a type meeting \added{the} \tmpnewoldconcept{MutableBufferSequence}~(\ref{buffer.reqmts.mutablebuffersequence}) requirements. The mutable buffer sequence specifies memory where the data should be placed. A read operation \changed{shall always fill}{fills} a buffer in the sequence completely before proceeding to the next.

\pnum
A \defn{write operation} is an operation that writes data from a constant buffer sequence argument of a type meeting \added{the} \tmpnewoldconcept{ConstBufferSequence}~(\ref{buffer.reqmts.constbuffersequence}) requirements. The constant buffer sequence specifies memory where the data to be written is located. A write operation \changed{shall always write}{writes} a buffer in the sequence completely before proceeding to the next.

\indextext{asynchronous operation}%
\pnum
If a read or write operation is also an asynchronous operation~(\ref{async.reqmts.async}), the operation \changed{shall maintain}{maintains} one or more copies of the buffer sequence until such time as the operation no longer requires access to the memory specified by the buffers in the sequence. The program shall ensure the memory remains valid until:
...
\pnum
The \tcode{mutable_buffer} class \changed{satisfies}{meets} the requirements of \tmpnewoldconcept{MutableBufferSequence}~(\ref{buffer.reqmts.mutablebuffersequence}), \tmpoldconcept{Default\-Cons\-tr\-uct\-ible} (\CppXref{defaultconstructible}), and \tmpoldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\pnum
The \tcode{const_buffer} class \changed{satisfies}{meets} requirements of \tmpnewoldconcept{ConstBufferSequence}~(\ref{buffer.reqmts.constbuffersequence}), \tmpoldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), and \tmpoldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\pnum
This subclause contains templates that may be used to query the properties of a type at compile time. Each of these templates is a \changed{UnaryTypeTrait}{\oldconcept{UnaryTypeTrait}} (\CppXref{meta.rqmts}) with a \changed{BaseCharacteristic}{base characteristic} of \tcode{true_type} if the corresponding condition is true, otherwise \tcode{false_type}.
...
\changed{an argument of type \tcode{vector<>}}{a specialization of \tcode{vector}},
...
\pnum
For the function overloads below that accept
\changed{an argument of type \tcode{basic_string<>}}{a specialization of \tcode{basic_string}},
...
\pnum
The \tcode{dynamic_vector_buffer} class template meets the requirements of \tmpnewoldconcept{DynamicBuffer}~(\ref{buffer.reqmts.dynamicbuffer}).

\pnum
The \tcode{dynamic_vector_buffer} class template requires that \tcode{T} is a trivially copyable or standard-layout type (\CppXref{basic.types}) and that \tcode{sizeof(T) == 1}\added{, otherwise the program is ill-formed}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{vec.size() <= maximum_size}.
...
\pnum
\changed{\remarks}{\throws} \tcode{length_error} if \tcode{size() + n} exceeds \tcode{max_size()}.
\end{itemdescr}
...
\pnum
The \tcode{dynamic_string_buffer} class template meets the requirements of \tmpnewoldconcept{DynamicBuffer}~(\ref{buffer.reqmts.dynamicbuffer}).

\pnum
The \tcode{dynamic_string_buffer} class template requires that \tcode{sizeof(CharT) == 1}\added{, otherwise the program is ill-formed}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{str.size() <= maximum_size}.
