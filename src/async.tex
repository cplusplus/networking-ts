...
\tcode{X u(cx1);}  &
  &
Shall not exit via an exception.\br\textit{post:} \tcode{u == cx1} and \tcode{std::addressof(u.context()) == std::addressof(cx1.context()).}  \\ \rowsep
...
\tcode{cx1 == cx2}  &
\tcode{bool}  &
Returns \tcode{true} only if \tcode{cx1} and \tcode{cx2} can be interchanged with identical effects in any of the expressions defined in these type requirements. \enternote Returning \tcode{false} does not necessarily imply that the effects are not identical. \exitnote\br\tcode{operator==} shall be reflexive, symmetric, and transitive, and shall not exit via an exception.  \\ \rowsep
