...
@\removed{inline }@namespace @\added{std::experimental::net::inline}@ @\namespacever@ {
  ...
  template<class T>
    @\added{inline}@ constexpr bool is_executor_v = is_executor<T>::value;

  struct executor_arg_t { @\added{explicit executor_arg_t() = default;}@ };
  @\added{inline}@ constexpr executor_arg_t executor_arg = executor_arg_t();

  template<class T, class Executor> struct uses_executor;

  template<class T, class Executor>
    @\added{inline}@ constexpr bool uses_executor_v = uses_executor<T, Executor>::value;
  ...
  @\removed{bool operator!=(const system_executor\&, const system_executor\&);}@
  ...
  @\removed{bool operator==(nullptr_t, const executor\& e) noexcept;}@
  @\removed{bool operator!=(const executor\& a, const executor\& b) noexcept;}@
  @\removed{bool operator!=(const executor\& e, nullptr_t) noexcept;}@
  @\removed{bool operator!=(nullptr_t, const executor\& e) noexcept;}@
  ...
  @\removed{template<class Executor>}@
    @\removed{bool operator!=(const strand<Executor>\& a, const strand<Executor>\& b);}@
...
} // inline namespace \added{experimental::net::}\namespacever
@\removed{\} // namespace net}@
@\removed{\} // namespace experimental}@

  template<class Allocator>
    struct uses_allocator<experimental::net::@\namespacever@::executor, Allocator>
      : true_type {};
...
\pnum
A type \tcode{A} meets the \changed{proto-allocator}{\defnnewoldconcept{ProtoAllocator}} requirements if \tcode{A} \changed{is}{meets the requirements of} \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), \tmpoldconcept{Destructible} (\CppXref{destructible}), and \tcode{allocator_traits<A>::rebind_alloc<U>} meets the \changed{allocator}{\oldconcept{Allocator}} requirements (\CppXref{allocator.requirements}), where \tcode{U} is \changed{an}{a \cv-unqualified} object type. \begin{note} For example, \tcode{allocator<void>} meets the proto-allocator requirements but not the allocator requirements. \end{note} No comparison operator, copy operation, move operation, or swap operation on these types shall exit via an exception.

\ednote{Restored "shall exit" above.}


\rSec2[async.reqmts.executor]{Executor requirements}

\pnum
The library describes a standard set of requirements for executors. A type meeting the \tmpdefnnewoldconcept{Executor} requirements embodies a set of rules for determining how submitted function objects are to be executed.

\pnum
A type \tcode{X} meets the \tmpnewoldconcept{Executor} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}) and \tmpoldconcept{Destructible} (\CppXref{destructible}), as well as the additional requirements listed below.
...
\ednote{Restored "shall exit" and "shall not" above.}
...
\tcode{x1} and \tcode{x2} denote \removed{(possibly const)} values of type \added{(possibly const)} \tcode{X},
\tcode{mx1} denotes an xvalue of type \tcode{X},
\tcode{f} denotes a function object of \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}) type \tcode{Func} such that \tcode{f()} is a valid expression,
\tcode{a} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{A}
where \tcode{A} is a type meeting the \tmpnewoldconcept{ProtoAllocator} requirements (\ref{async.reqmts.proto.allocator}),
and \tcode{u} denotes an identifier.

\begin{libreqtab3}
{\tmpnewoldconcept{Executor} requirements}
...
\tcode{X u(x1);}  &
  &
Shall not exit via an exception.\br\changed{\textit{post:}}{\postconditions} \tcode{u == x1} and \tcode{\removed{std::}addressof(u.context()) == \removed{std::}addressof(x1.context()).}  \\ \rowsep

\tcode{X u(mx1);}  &
  &
Shall not exit via an exception.\br\changed{\textit{post:}}{\postconditions} \tcode{u} equals the prior value of \tcode{mx1} and \tcode{\removed{std::}addressof(u.context())} equals the prior value of \tcode{\removed{std::}addressof(mx1.context())}.  \\ \rowsep
...
\tcode{x1.context()}  &
\tcode{execution_context\&}, or \tcode{E\&} where \tcode{E} is a type that \changed{satisfies}{meets} the \tmpnewoldconcept{ExecutionContext} ~(\ref{async.reqmts.executioncontext}) requirements.  &
...
\tcode{x1.on_work_finished()}  &
  &
Shall not exit via an exception. \br\changed{\textit{Precondition:}}{\expects} A preceding call \tcode{x2.on_work_started()} where \tcode{x1 == x2}.  \\ \rowsep

\tcode{x1.dispatch(\br\mbox{\ \ }std::move(f),~a)}  &
  &
\effects Creates an object \tcode{f1} initialized with \exposid{\changed{DECAY_COPY}{decay-copy}}\tcode{(std::forward<Func>(f))} (\CppXref{expos.only.func}) in the current thread of execution. Calls \tcode{f1()} at most once. The executor may block forward progress of the caller until \tcode{f1()} finishes execution. Executor implementations should use the supplied allocator to allocate any memory required to store the function object. Prior to invoking the function object, the executor shall deallocate any memory allocated. \begin{note} Executors defined in this document always use the supplied allocator unless otherwise specified. \end{note} \br
...
\tcode{x1.post(std::move(f),~a)}  &
  &
\effects Creates an object \tcode{f1} initialized with \exposid{\changed{DECAY_COPY}{decay-copy}}\tcode{(std::forward<Func>(f))} in the current thread of execution. Calls \tcode{f1()} at most once. The executor shall not block forward progress of the caller pending completion of \tcode{f1()}. The executor may begin \tcode{f1}'s progress before the call to \tcode{post} completes. Executor implementations should use the supplied allocator to allocate any memory required to store the function object. Prior to invoking the function object, the executor shall deallocate any memory allocated. \begin{note} Executors defined in this document always use the supplied allocator unless otherwise specified. \end{note}\br
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{ExecutionContext} requirements if it is publicly and unambiguously derived from \tcode{execution_context}, and \changed{satisfies}{meets} the additional requirements listed below.

\pnum
In Table~\ref{tab:async.reqmts.executioncontext.requirements}, \tcode{x} denotes a value of type \tcode{X}.

\indextext{requirements!\idxnewoldconcept{ExecutionContext}}%
\indextext{\idxnewoldconcept{ExecutionContext}}%
\begin{libreqtab3}
{\tmpnewoldconcept{ExecutionContext} requirements}
...
type meeting \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}) requirements  &
  \\ \rowsep
...
\pnum
\changed{A class is a service}{A \defn{service} is a class type that meets the \newoldconcept{Service} requirements. A type meets the \defnnewoldconcept{Service} requirements}
if it is publicly and unambiguously derived from \tcode{execution_context::service}, or if it is publicly and unambiguously derived from another service.
For a service \tcode{S}, \tcode{S::key_type} shall be valid and denote a type (\CppXref{temp.deduct}), \tcode{is_base_of_v<typename S::key_type, S>} shall be \tcode{true}, and \tcode{S} shall \changed{satisfy}{meet} the \tmpoldconcept{Destructible} requirements (\CppXref{destructible}).

\pnum
The first parameter of all service constructors shall be an lvalue reference to \tcode{execution_context}. This parameter denotes the \tcode{execution_context} object that represents a set of services, of which the service object will be a member. \begin{note} These constructors can be called by the \tcode{make_service} function. \end{note}

\pnum
A service shall provide an explicit constructor with a single parameter of lvalue reference to \tcode{execution_context}. \begin{note} This constructor can be called by the \tcode{use_service} function. \end{note}
...
\pnum
A type \changed{satisfies}{meets} the \changed{signature}{\defnnewoldconcept{Signature}} requirements if it is a call signature (\CppXref{func.def}).
...
\pnum
An \changed{associator}{\defn{associator}} defines a relationship between different types and objects where, given:
...
\item
a candidate object \tcode{c} of type \tcode{C} meeting the type requirements \newoldconcept{R},
\end{itemize}

an associated type \tcode{A} meeting the type requirements \newoldconcept{R} may be computed, and an associated object \tcode{a} of type \tcode{A} may be obtained.

\pnum
An associator shall be a class template that takes two template type arguments. The first template argument is the source type \tcode{S}. The second template argument is the candidate type \tcode{C}. The second template argument shall be defaulted to some default candidate type \tcode{D} that \changed{satisfies}{meets} the type requirements \newoldconcept{R}.

\pnum
An associator shall additionally \changed{satisfy}{meet} the requirements in Table~\ref{tab:async.reqmts.associator.requirements}.
...
\tcode{c} is a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{C},
...
and \tcode{d} is a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{D} that is the default candidate object.

\begin{libreqtab3}
{\tmpnewoldconcept{Associator} requirements}
...
\pnum
The associator's primary template shall be defined. A program may partially specialize the associator class template for some \changed{user}{program}-defined type \tcode{S}.
...
\pnum
This subclause uses the names \tcode{Alloc1}, \tcode{Alloc2}, \tcode{alloc1}, \tcode{alloc2}, \tcode{Args}, \tcode{CompletionHandler}, \tcode{completion_handler}, \tcode{Executor1}, \tcode{Executor2}, \tcode{ex1}, \tcode{ex2}, \tcode{f}, \placeholder{i}, \placeholder{N}, \tcode{Signature}, \tcode{token}, \tcode{T}$_i$, \tcode{t}$_i$, \tcode{work1}, and \tcode{work2} as placeholders for specifying the requirements below.
...
\pnum
In this document, all functions with the prefix \tcode{async_} are initiating functions.
...
\ednote{This subclause doesn't actually define any requirements for
\tcode{CompletionToken}. I experimented with introducing an exposition-only
alias template \exposid{completion-handler-type} to obtain the
\tcode{CompletionHandler} type
from \tcode{CompletionToken} and \tcode{Signature},
but it's tricky to specify clearly.
A type cannot be a \tcode{CompletionToken} by itself,
it can only be a completion token \textit{for a given call signature}.
The requirement is that a type \tcode{CompletionHandler} can be obtained from
\exposid{completion-handler-type}\tcode{<CompletionToken, Signature>}
and that \tcode{CompletionHandler} is
destructible, move constructible, constructible from \tcode{CompletionToken},
and callable with the call signature \tcode{Signature}.
}
...
\pnum
The type \tcode{CompletionHandler} shall \changed{satisfy}{meet} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}) and \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}), and be callable with the specified call signature.
...
\pnum
For the sake of exposition, this document sometimes annotates functions with a return type \DEDUCED. For every function declaration that returns \DEDUCED, the meaning is equivalent to specifying the return type as \tcode{typename async_result<decay_t<CompletionToken>, Signature>::return_type}.
...
\pnum
 Unless otherwise specified, the lifetime of arguments to initiating functions \changed{shall be}{is} treated as follows:
...
\item
Otherwise, the implementation does not assume the validity of the argument after the initiating function completes. \begin{note} In other words, the program is not required to guarantee the validity of the argument after the initiating function completes. \end{note} The implementation may make copies of the argument, and all copies \changed{shall be}{are} destroyed no later than immediately after invocation of the completion handler.
\end{itemize}
...
\pnum
 An initiating function \changed{shall}{does} not block (\CppXref{defns.block}) the calling thread pending completion of the outstanding operation.
...
\pnum
An asynchronous operation has an associated executor \changed{satisfying}{meeting} the \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}) requirements. If not otherwise specified by the asynchronous operation, this associated executor is an object of type \tcode{system_executor}.
...
\pnum
Until the asynchronous operation has completed, the asynchronous operation \changed{shall maintain}{maintains}:
...
\pnum
Let \tcode{Alloc1} be a type, \changed{satisfying}{meeting} the \tmpnewoldconcept{ProtoAllocator}~(\ref{async.reqmts.proto.allocator}) requirements, that represents the asynchronous operation's default allocation strategy. \begin{note} Typically \tcode{allocator<void>}. \end{note} Let \tcode{alloc1} be a value of type \tcode{Alloc1}.

\pnum
A completion handler object of type \tcode{CompletionHandler} has an associated allocator object \tcode{alloc2} of type \tcode{Alloc2} \changed{satisfying}{meeting} the \tmpnewoldconcept{ProtoAllocator}~(\ref{async.reqmts.proto.allocator}) requirements. The type \tcode{Alloc2} is \tcode{associated_allocator_t<CompletionHandler, Alloc1>}. Let \tcode{alloc2} be a value of type \tcode{Alloc2} obtained by performing \tcode{get_associated_allocator(completion_handler, alloc1)}.
...
\pnum
 \begin{note} The implementation can perform operating system or underlying API calls that perform memory allocations not using the associated allocator. Invocations of the allocator functions do not introduce data races (see \CppXref{res.on.data.races}). \end{note}
...
\pnum
If an asynchronous operation completes immediately (that is, within the thread of execution calling the initiating function, and before the initiating function returns), the completion handler \changed{shall be}{is} submitted for execution as if by performing \tcode{ex2.post(std::move(f), alloc2)}. Otherwise, the completion handler \changed{shall be}{is} submitted for execution as if by performing \tcode{ex2.dispatch(std::move(f), alloc2)}.
...
\item
an object of an unspecified type \changed{satisfying}{meeting} the changed{Executor}{\newoldconcept{Executor}} requirements~(\ref{async.reqmts.executor}), that delegates executor operations to the type \tcode{Executor2} and object \tcode{ex2}.
\end{itemize}

An intermediate operation's completion handler \changed{shall have}{has} an associated allocator that is either:
...
\item
an object of an unspecified type \changed{satisfying}{meeting} the \changed{ProtoAllocator}{\newoldconcept{ProtoAllocator}} requirements~(\ref{async.reqmts.proto.allocator}), that delegates allocator operations to the type \tcode{Alloc2} and object \tcode{alloc2}.
\end{itemize}
...
\tcode{R::completion_handler_type}  &
  &
A type \changed{satisfying}{meeting} \tmpoldconcept{MoveConstructible} requirements (\CppXref{moveconstructible}). An object of type \tcode{completion_handler_type} shall be a function object with call signature \tcode{Signature}, and \tcode{completion_handler_type} shall be constructible with an rvalue of type \tcode{CompletionToken}.  \\ \rowsep

\tcode{R::return_type}  &
  &
\tcode{void}; or a type \changed{satisfying}{meeting} \tmpoldconcept{MoveConstructible} requirements (\CppXref{moveconstructible})  \\ \rowsep
...
\pnum
Specializations of \tcode{associated_allocator} shall \changed{satisfy}{meet} the requirements in Table~\ref{tab:async.assoc.alloc.requirements}.
...
\tcode{t} is a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{T};
and \tcode{a} is an object of type \tmpnewoldconcept{ProtoAllocator}.
...
\begin{itemdescr}
\pnum
\effects Creates an object of class \tcode{execution_context} which contains no services. \begin{note} An implementation can preload services of internal service types for its own use. \end{note}
\end{itemdescr}
...
\begin{itemdescr}
\ednote{This is not a precondition, because the \throws paragraph defines it.}

\pnum
\removed{\requires A service object of type \tcode{Service::key_type} does not already exist in the \tcode{execution_context} set identified by \tcode{ctx}.}
...
\pnum
The class template \tcode{is_executor} can be used to detect executor types \changed{satisfying}{meeting} the \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}) type requirements.
...
\pnum
\added{The template argument} \tcode{T} shall be a complete type.

\pnum
Class template \tcode{is_executor} is a \changed{UnaryTypeTrait}{\oldconcept{UnaryTypeTrait}} (\CppXref{meta.rqmts}) with a \changed{BaseCharacteristic}{base characteristic} of \tcode{true_type} if the type \tcode{T} meets the syntactic requirements for \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}), otherwise \tcode{false_type}.
...
\pnum
The \tcode{executor_arg_t} struct is an empty structure type used as a unique type to disambiguate constructor and function overloading. Specifically, types may have constructors with \tcode{executor_arg_t} as the first argument, immediately followed by an argument of a type that \changed{satisfies}{meets} the \changed{Executor}{\newoldconcept{Executor}} requirements~(\ref{async.reqmts.executor}).
...
\pnum
\changed{Remark:}{\remarks} Detects whether \tcode{T} has a nested \tcode{executor_type} that is convertible from \tcode{Executor}. Meets the \tmpoldconcept{BinaryTypeTrait} requirements (\CppXref{meta.rqmts}). The implementation provides a definition that \changed{is derived from}{has a base characteristic of} \tcode{true_type} if the \grammarterm{qualified-id} \tcode{T::executor_type} is valid and denotes a type and \tcode{is_convertible\added{_v}<Executor, T::executor_type>\removed{::value} != false}, otherwise it \changed{is derived from}{has a base characteristic of} \tcode{false_type}. A program may specialize this template to derive from \tcode{true_type} for a \changed{user}{program}-defined type \tcode{T} that does not have a nested \tcode{executor_type} but nonetheless can be constructed with an executor if the first argument of a constructor has type \tcode{executor_arg_t} and the second argument has type \tcode{Executor}.
...
and \tcode{is_constructible\added{_v}<T, executor_arg_t, Executor, V1, V2, ..., VN>\removed{::value}} is \tcode{true},
...
\pnum
Class template \tcode{associated_executor} is an associator~(\ref{async.reqmts.associator}) for the \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}) type requirements, with default candidate type \tcode{system_executor} and default candidate object \tcode{system_executor()}.
...
\pnum
Specializations of \tcode{associated_executor} shall \changed{satisfy}{meet} the requirements in Table~\ref{tab:async.assoc.exec.requirements}.
In this table, \tcode{X} is a specialization of \tcode{associated_executor}
for the template parameters \tcode{T} and \tcode{Executor};
\tcode{t} is a \removed{(possible const)} value of \added{type (possibly const)} \tcode{T};
...
\pnum
\changed{\remarks This function shall not participate in overload resolution unless}
{\constraints{}}
\tcode{is_convertible\added{_v}<Exec\-ution\-Context\&, execution_context\&>\removed{::value}} is \tcode{true}.
\end{itemdescr}
...
an executor of type \tcode{Executor} \changed{satisfying}{meeting} the \changed{Executor}{\newoldconcept{Executor}} requirements~(\ref{async.reqmts.executor})
...
@\removed{inline }@namespace @\added{std::experimental::net::inline}@ @\namespacever@ {
  ...
  public:
    ...
    template<class... Args>
      @\changed{result_of_t<T\&(Args\&\&...)>}{invoke_result_t<T\&, Args...>}@ operator()(Args&&... args);
    template<class... Args>
      @\changed{result_of_t<const T\&(Args\&\&...)>}{invoke_result_t<const T\&, Args...>}@ operator()(Args&&... args) const;
...
\ednote{
There was a comment in the LWG review that 
"for some of the [async.exec.binder.access] we could use constructor delegation"
but I'm unsure what the suggestion was. It can probably be done editorially.
}
...
\mandates
\tcode{is_convertible_v<OtherExecutor, Executor>} is \tcode{true}
and
uses-executor construction of \tcode{T} with executor of type \tcode{Executor}
and a constructor argument of type \tcode{const U\&} is well-formed.
}

\ednote{
Fixed value category above, \tcode{const U\&} rather than \tcode{U}.
Replaced \tcode{is_constructible<v<U,T>} with uses-executor construction.
Could spell it out as:
\tcode{uses_executor_v<T, Executor>} is \tcode{true} and
  \tcode{is_constructible_v<T, executor_arg_t, Executor, const U\&>}
  is \tcode{true}, or
...
and
uses-executor construction of \tcode{T} with executor of type \tcode{Executor}
and a constructor argument of type \tcode{U\&\&} is well-formed.
}
...
\mandates \tcode{is_convertible_v<U, T>} is \tcode{true}.
}
...
\begin{itemdescr}
\ednote{Failing to meet this \requires was undefined, not ill-formed, but I'm changing it to \mandates because that seems right.}

\pnum
\changed{%
\requires \tcode{U} is \tcode{T} or convertible to \tcode{T}.
}{%
\mandates \tcode{is_same_v<U, T>} is \tcode{true}
or \tcode{is_convertible_v<U, T>} is \tcode{true}.
}
...
\added{%
\effects Equivalent to: \tcode{return bind_executor(ctx.get_executor(), forward<T>(t))}.
}
...
and \tcode{is_convertible\added{_v}<T\&, execution_context\&>\removed{::value}} is \tcode{false}.
\end{itemdescr}
...
@\removed{inline }@namespace @\added{std::experimental::net::inline}@ @\namespacever@ {
  ...
  @\removed{bool operator!=(const system_executor\&, const system_executor\&) noexcept;}@
...
\pnum
Class \tcode{system_executor} \changed{satisfies}{meets} the \tmpoldconcept{Destructible} (\CppXref{destructible}), \tmpoldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), and \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}) type requirements.

\pnum
To \changed{satisfy}{meet} the \tmpnewoldconcept{Executor} requirements for the \tcode{post} and \tcode{defer} member functions, the system executor may create \tcode{thread} objects to run the submitted function objects. These \tcode{thread} objects are collectively referred to as system threads.
...
\begin{itemdescr}
\pnum
\effects Equivalent to \exposid{\changed{DECAY_COPY}{decay-copy}}\tcode{(forward<Func>(f))()} (\CppXref{expos.only.func}).
\end{itemdescr}
...
creates an object \tcode{f1} initialized with \exposid{\changed{DECAY_COPY}{decay-copy}}\tcode{(forw\-ard<Func>(f))},
and calls \tcode{f1()} as if in a thread of execution represented by a \tcode{thread} object.
Any exception propagated from the
\changed{execution of \exposid{DECAY_COPY}\tcode{(forward<Func>(f))()}}{initialization of \tcode{f1} or
evaluating
\tcode{f1()}}
...
\pnum
The class \tcode{system_context} \changed{satisfies}{meets} the \tmpnewoldconcept{ExecutionContext}~(\ref{async.reqmts.executioncontext}) type requirements.
...
\pnum
The \tcode{executor} class provides a polymorphic wrapper for types that \changed{satisfy}{meet} the \changed{Executor}{\newoldconcept{Executor}} requirements~(\ref{async.reqmts.executor}).
...
\pnum
Class \tcode{executor} meets the requirements of \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}), \tmpoldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), and \tmpoldconcept{CopyAssignable} (\CppXref{copyassignable}).

\pnum
\begin{note} To meet the \tcode{noexcept} requirements for executor copy constructors and move constructors, implementations can share a target between two or more \tcode{executor} objects. \end{note}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{*this != nullptr}.
...
\begin{itemdescr}
\pnum
Let \tcode{e} be the target object of \tcode{*this}. Let \tcode{a1} be the allocator that was specified when the target was set. Let \tcode{fd} be the result of \exposid{\changed{DECAY_COPY}{decay-copy}}\tcode{(f)} (\CppXref{expos.only.func}).
...
\begin{itemdescr}
\pnum
Let \tcode{e} be the target object of \tcode{*this}. Let \tcode{a1} be the allocator that was specified when the target was set. Let \tcode{fd} be the result of \exposid{\changed{DECAY_COPY}{decay-copy}}\tcode{(f)}.
...
\pnum
\begin{note} The function \tcode{dispatch} \changed{satisfies}{meets} the requirements for an asynchronous operation~(\ref{async.reqmts.async}), except for the requirement that the operation uses \tcode{post} if it completes immediately. \end{note}
...
\pnum
\begin{note} The function \tcode{post} \changed{satisfies}{meets} the requirements for an asynchronous operation~(\ref{async.reqmts.async}). \end{note}
...
\pnum
\begin{note} The function \tcode{defer} \changed{satisfies}{meets} the requirements for an asynchronous operation~(\ref{async.reqmts.async}), except for the requirement that the operation uses \tcode{post} if it completes immediately. \end{note}
...
\pnum
The class template \tcode{strand} is a wrapper around an object of type \tcode{Executor} \changed{satisfying}{meeting} the \changed{Executor}{\newoldconcept{Executor}} requirements~(\ref{async.reqmts.executor}).
...
\pnum
\tcode{strand<Executor>} \changed{satisfies}{meets} the \tmpnewoldconcept{Executor}~(\ref{async.reqmts.executor}) requirements.
...
\pnum
\removed{\remarks This overload shall not participate in overload resolution unless
\tcode{Executor} satisfies the \tcode{DefaultConstructible} requirements (\CppXref{defaultconstructible}).}

\added{%
\constraints \tcode{is_default_constructible_v<Executor>} is \tcode{true}.
}
\end{itemdescr}
...
\item
\tcode{get_inner_executor() == other.get_inner_executor()} \ednote{Really? RHS is moved from}
\end{itemize}
\end{itemdescr}
...
\ednote{The next two constructors are changed to be constrained,
so that \tcode{is_constructible} gives the right answer.}
...
\added{%
\constraints \tcode{is_convertible_v<const OtherExecutor\&, Executor>} is \tcode{true}.
}
...
\added{%
\constraints \tcode{is_convertible_v<OtherExecutor, Executor>} is \tcode{true}.
}
...
\added{%
\pnum
\mandates \tcode{is_copy_assignable_v<Executor>} is \tcode{true}.
}

\pnum
\changed{\requires}{\expects} \tcode{Executor} is \tcode{CopyAssignable} (\CppXref{copyassignable}).
...
\added{%
\pnum
\mandates \tcode{is_move_assignable_v<Executor>} is \tcode{true}.
}

\pnum
\changed{\requires}{\expects} \tcode{Executor} is \tcode{MoveAssignable} (\CppXref{moveassignable}).
...
\begin{itemdescr}
\pnum
\changed{\requires \tcode{OtherExecutor} is convertible to \tcode{Executor}. \tcode{Executor} is \tcode{CopyAssignable} (\CppXref{copyassignable}).}{
\mandates \tcode{is_convertible_v<OtherExecutor, Executor>} is \tcode{true}
and \tcode{is_copy_assignable_v<Executor>} is \tcode{true}.}

\pnum
\effects Assigns \tcode{other.inner_ex_} to \tcode{inner_ex_}.
\ednote{The \requires (and now \mandates) suggest this should be \tcode{inner_ex_ = Executor(other.inner_ex_)} instead.}
...
\begin{itemdescr}
\pnum
\changed{\requires \tcode{OtherExecutor} is convertible to \tcode{Executor}. \tcode{Executor} is \tcode{MoveAssignable} (\CppXref{moveassignable}).}{
\mandates \tcode{is_convertible_v<OtherExecutor, Executor>} is \tcode{true}
and \tcode{is_move_assignable_v<Executor>} is \tcode{true}.}

\pnum
\effects Assigns \tcode{std::move(other.inner_ex_)} to \tcode{inner_ex_}.
\ednote{The \requires (and now \mandates) suggest this should be \tcode{inner_ex_ = Executor(std::move(other.inner_ex_))} instead.}
...
\begin{itemdescr}
\pnum
\effects \removed{Destroys an object of class \tcode{strand<Executor>}.}
...
\pnum
The class template \tcode{use_future_t} defines a set of types that, when passed as a completion token~(\ref{async.reqmts.async.token}) to an asynchronous operation's initiating function, cause the result of the asynchronous operation to be delivered via a \changed{future}{\tcode{future}}~(\CppXref{futures.uniquefuture}).
...
@\removed{inline }@namespace @\added{std::experimental::net::inline}@ @\namespacever@ {
  ...
  public:
    ...
    constexpr use_future_t() noexcept(@\changed{noexcept(allocator_type())}{is_nothrow_default_constructible_v<allocator_type>}@);
...
\begin{itemdescr}
\ednote{Allocators don't have to be default constructible.}

\begin{addedblock}
\pnum
\mandates \tcode{is_default_constructible_v<allocator_type>} is \tcode{true}.
\end{addedblock}
...
\pnum
\remarks The return type \tcode{T} \changed{satisfies}{meets} the \tmpoldconcept{Destructible} (\CppXref{destructible}) and \tmpoldconcept{MoveConstructible} (\CppXref{moveconstructible}) requirements.
...
\item
 the nested type \tcode{return_type} is \tcode{future<\changed{result_of_t<FD(decay_t<Args>...)>}{invoke_result_t<FD\&, Args...>}>}; and
\item
