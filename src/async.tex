...
\tcode{cx1 == cx2}  &
\tcode{bool}  &
Returns \tcode{true} only if \tcode{cx1} and \tcode{cx2} can be interchanged with identical effects in any of the expressions defined in these type requirements. \begin{note} Returning \tcode{false} does not necessarily imply that the effects are not identical. \end{note}\br\tcode{operator==} shall be reflexive, symmetric, and transitive, and shall not exit via an exception.  \\ \rowsep
