...
namespace std::experimental::net::inline @\namespacever@ {
  ...
  inline constexpr executor_arg_t executor_arg = executor_arg_t();

  template<class T, class Executor> struct uses_executor;

  template<class T, class Executor>
    inline constexpr bool uses_executor_v = uses_executor<T, Executor>::value;
...
} // inline namespace experimental::net::\namespacever
...
\pnum
A type \tcode{A} meets the \defnnewoldconcept{ProtoAllocator} requirements if \tcode{A} meets the requirements of \oldconcept{CopyConstructible} (\CppXref{copyconstructible}), \oldconcept{Destructible} (\CppXref{destructible}), and \tcode{allocator_traits<A>::rebind_alloc<U>} meets the \oldconcept{Allocator} requirements (\CppXref{allocator.requirements}), where \tcode{U} is a \cv-unqualified object type. \begin{note} For example, \tcode{allocator<void>} meets the proto-allocator requirements but not the allocator requirements. \end{note} No comparison operator, copy operation, move operation, or swap operation on these types shall exit via an exception.


\rSec2[async.reqmts.executor]{Executor requirements}

\pnum
The library describes a standard set of requirements for executors. A type meeting the \defnnewoldconcept{Executor} requirements embodies a set of rules for determining how submitted function objects are to be executed.

\pnum
A type \tcode{X} meets the \newoldconcept{Executor} requirements if it meets the requirements of \oldconcept{CopyConstructible} (\CppXref{copyconstructible}) and \oldconcept{Destructible} (\CppXref{destructible}), as well as the additional requirements listed below.
...
\tcode{x1} and \tcode{x2} denote values of type (possibly const) \tcode{X},
\tcode{mx1} denotes an xvalue of type \tcode{X},
\tcode{f} denotes a function object of \oldconcept{MoveConstructible} (\CppXref{moveconstructible}) type \tcode{Func} such that \tcode{f()} is a valid expression,
\tcode{a} denotes a value of type (possibly const) \tcode{A}
where \tcode{A} is a type meeting the \newoldconcept{ProtoAllocator} requirements (\ref{async.reqmts.proto.allocator}),
and \tcode{u} denotes an identifier.

\begin{libreqtab3}
{\newoldconcept{Executor} requirements}
...
\tcode{X u(x1);}  &
  &
Shall not exit via an exception.\br\postconditions{} \tcode{u == x1} and \tcode{addressof(u.context()) == addressof(x1.context()).}  \\ \rowsep

\tcode{X u(mx1);}  &
  &
Shall not exit via an exception.\br\postconditions{}  \tcode{u} equals the prior value of \tcode{mx1} and \tcode{addressof(u.context())} equals the prior value of \tcode{addressof(mx1.context())}.  \\ \rowsep
...
\tcode{x1.context()}  &
\tcode{execution_context\&}, or \tcode{E\&} where \tcode{E} is a type that meets the \newoldconcept{ExecutionContext} ~(\ref{async.reqmts.executioncontext}) requirements.  &
...
\tcode{x1.on_work_finished()}  &
  &
Shall not exit via an exception. \br\expects{} A preceding call \tcode{x2.on_work_started()} where \tcode{x1 == x2}.  \\ \rowsep

\tcode{x1.dispatch(\br\mbox{\ \ }std::move(f),~a)}  &
  &
\effects Creates an object \tcode{f1} initialized with \exposid{decay-copy}\tcode{(std::forward<Func>(f))} (\CppXref{expos.only.func}) in the current thread of execution. Calls \tcode{f1()} at most once. The executor may block forward progress of the caller until \tcode{f1()} finishes execution. Executor implementations should use the supplied allocator to allocate any memory required to store the function object. Prior to invoking the function object, the executor shall deallocate any memory allocated. \begin{note} Executors defined in this document always use the supplied allocator unless otherwise specified. \end{note} \br
...
\tcode{x1.post(std::move(f),~a)}  &
  &
\effects Creates an object \tcode{f1} initialized with \exposid{decay-copy}\tcode{(std::forward<Func>(f))} in the current thread of execution. Calls \tcode{f1()} at most once. The executor shall not block forward progress of the caller pending completion of \tcode{f1()}. The executor may begin \tcode{f1}'s progress before the call to \tcode{post} completes. Executor implementations should use the supplied allocator to allocate any memory required to store the function object. Prior to invoking the function object, the executor shall deallocate any memory allocated. \begin{note} Executors defined in this document always use the supplied allocator unless otherwise specified. \end{note}\br
...
\pnum
A type \tcode{X} meets the \defnnewoldconcept{ExecutionContext} requirements if it is publicly and unambiguously derived from \tcode{execution_context}, and meets the additional requirements listed below.
...
\begin{libreqtab3}
{\newoldconcept{ExecutionContext} requirements}
...
type meeting \newoldconcept{Executor}~(\ref{async.reqmts.executor}) requirements  &
  \\ \rowsep
...
For a service \tcode{S}, \tcode{S::key_type} shall be valid and denote a type (\CppXref{temp.deduct}), \tcode{is_base_of_v<typename S::key_type, S>} shall be \tcode{true}, and \tcode{S} shall meet the \oldconcept{Destructible} requirements (\CppXref{destructible}).
...
\pnum
A type meets the \defnnewoldconcept{Signature} requirements if it is a call signature (\CppXref{func.def}).
...
\pnum
An \defn{associator} defines a relationship between different types and objects where, given:
...
\pnum
An associator shall be a class template that takes two template type arguments. The first template argument is the source type \tcode{S}. The second template argument is the candidate type \tcode{C}. The second template argument shall be defaulted to some default candidate type \tcode{D} that meets the type requirements \newoldconcept{R}.

\pnum
An associator shall additionally meet the requirements in Table~\ref{tab:async.reqmts.associator.requirements}.
...
\tcode{c} is a  value of type (possibly const) \tcode{C},
...
and \tcode{d} is a  value of type (possibly const) \tcode{D} that is the default candidate object.

\begin{libreqtab3}
{\newoldconcept{Associator} requirements}
...
\pnum
The associator's primary template shall be defined. A program may partially specialize the associator class template for some program-defined type \tcode{S}.
...
\pnum
The type \tcode{CompletionHandler} shall meet the requirements of \oldconcept{Destructible} (\CppXref{destructible}) and \oldconcept{MoveConstructible} (\CppXref{moveconstructible}), and be callable with the specified call signature.
...
\pnum
 Unless otherwise specified, the lifetime of arguments to initiating functions is treated as follows:
...
\pnum
 An initiating function does not block (\CppXref{defns.block}) the calling thread pending completion of the outstanding operation.
...
\pnum
An asynchronous operation has an associated executor meeting the \newoldconcept{Executor}~(\ref{async.reqmts.executor}) requirements. If not otherwise specified by the asynchronous operation, this associated executor is an object of type \tcode{system_executor}.
...
\pnum
Until the asynchronous operation has completed, the asynchronous operation maintains:
...
\pnum
Let \tcode{Alloc1} be a type, meeting the \newoldconcept{ProtoAllocator}~(\ref{async.reqmts.proto.allocator}) requirements, that represents the asynchronous operation's default allocation strategy. \begin{note} Typically \tcode{allocator<void>}. \end{note} Let \tcode{alloc1} be a value of type \tcode{Alloc1}.

\pnum
A completion handler object of type \tcode{CompletionHandler} has an associated allocator object \tcode{alloc2} of type \tcode{Alloc2} meeting the \newoldconcept{ProtoAllocator}~(\ref{async.reqmts.proto.allocator}) requirements. The type \tcode{Alloc2} is \tcode{associated_allocator_t<CompletionHandler, Alloc1>}. Let \tcode{alloc2} be a value of type \tcode{Alloc2} obtained by performing \tcode{get_associated_allocator(completion_handler, alloc1)}.
...
\pnum
If an asynchronous operation completes immediately (that is, within the thread of execution calling the initiating function, and before the initiating function returns), the completion handler is submitted for execution as if by performing \tcode{ex2.post(std::move(f), alloc2)}. Otherwise, the completion handler is submitted for execution as if by performing \tcode{ex2.dispatch(std::move(f), alloc2)}.
...
An intermediate operation's completion handler has an associated executor that is either:
...
\item
an object of an unspecified type meeting the changed{Executor}{\newoldconcept{Executor}} requirements~(\ref{async.reqmts.executor}), that delegates executor operations to the type \tcode{Executor2} and object \tcode{ex2}.
\end{itemize}

An intermediate operation's completion handler has an associated allocator that is either:
...
\tcode{R::completion_handler_type}  &
  &
A type meeting \oldconcept{MoveConstructible} requirements (\CppXref{moveconstructible}). An object of type \tcode{completion_handler_type} shall be a function object with call signature \tcode{Signature}, and \tcode{completion_handler_type} shall be constructible with an rvalue of type \tcode{CompletionToken}.  \\ \rowsep

\tcode{R::return_type}  &
  &
\tcode{void}; or a type meeting \oldconcept{MoveConstructible} requirements (\CppXref{moveconstructible})  \\ \rowsep
...
\pnum
Specializations of \tcode{associated_allocator} shall meet the requirements in Table~\ref{tab:async.assoc.alloc.requirements}.
...
\tcode{t} is a  value of type (possibly const) \tcode{T};
and \tcode{a} is an object of type \newoldconcept{ProtoAllocator}.
...
\rSec2[async.assoc.alloc.refwrap]{\tcode{associated_allocator<reference_wrapper<T>, ProtoAllocator>} members}

\begin{itemdecl}
type get(reference_wrapper<T> t, const ProtoAllocator& a = ProtoAllocator()) noexcept;
\end{itemdecl}

\begin{itemdescr}
\pnum
\returns \tcode{associated_allocator<T, ProtoAllocator>::get(t.get(), a)}.
\end{itemdescr}
...
\rSec2[async.exec.ctx.except]{Class \tcode{service_already_exists}}

\indexlibrary{\idxcode{service_already_exists}}%
\pnum The class \tcode{service_already_exists}
defines the type of objects thrown as exceptions to report
an attempt to add an existing service to an
...
\pnum
The class template \tcode{is_executor} can be used to detect executor types meeting the \newoldconcept{Executor}~(\ref{async.reqmts.executor}) type requirements.
...
\pnum
Class template \tcode{is_executor} is a \oldconcept{UnaryTypeTrait} (\CppXref{meta.rqmts}) with a base characteristic of \tcode{true_type} if the type \tcode{T} meets the syntactic requirements for \newoldconcept{Executor}~(\ref{async.reqmts.executor}), otherwise \tcode{false_type}.
...
\pnum
The \tcode{executor_arg_t} struct is an empty structure type used as a unique type to disambiguate constructor and function overloading. Specifically, types may have constructors with \tcode{executor_arg_t} as the first argument, immediately followed by an argument of a type that meets the \newoldconcept{Executor} requirements~(\ref{async.reqmts.executor}).
...
\pnum
\remarks Detects whether \tcode{T} has a nested \tcode{executor_type} that is convertible from \tcode{Executor}. Meets the \oldconcept{BinaryTypeTrait} requirements (\CppXref{meta.rqmts}). The implementation provides a definition that has a base characteristic of \tcode{true_type} if the \grammarterm{qualified-id} \tcode{T::executor_type} is valid and denotes a type and \tcode{is_convertible_v<Executor, T::executor_type>} is \tcode{true}, otherwise it has a base characteristic of \tcode{false_type}. A program may specialize this template to derive from \tcode{true_type} for a program-defined type \tcode{T} that does not have a nested \tcode{executor_type} but nonetheless can be constructed with an executor if the first argument of a constructor has type \tcode{executor_arg_t} and the second argument has type \tcode{Executor}.
...
and \tcode{is_constructible_v<T, executor_arg_t, Executor, V1, V2, ..., VN>} is \tcode{true},
...
\pnum
Class template \tcode{associated_executor} is an associator~(\ref{async.reqmts.associator}) for the \newoldconcept{Executor}~(\ref{async.reqmts.executor}) type requirements, with default candidate type \tcode{system_executor} and default candidate object \tcode{system_executor()}.
...
namespace std::experimental::net::inline @\namespacever@ {
  ...
  struct associated_executor<reference_wrapper<T>, Executor>
  {
    ...
    static type get(reference_wrapper<T> t, const Executor& e = Executor()) noexcept;
  };
...
\pnum
Specializations of \tcode{associated_executor} shall meet the requirements in Table~\ref{tab:async.assoc.exec.requirements}.
...
\rSec2[async.assoc.exec.refwrap]{\tcode{associated_executor<reference_wrapper<T>, ProtoAllocator>} members}
...
\begin{itemdescr}
\pnum
\returns \tcode{associated_executor<T, ProtoAllocator>::get(t.get(), e)}.
\end{itemdescr}
...
an executor of type \tcode{Executor} meeting the \newoldconcept{Executor} requirements~(\ref{async.reqmts.executor})
...
and \tcode{is_convertible_v<T\&, execution_context\&>} is \tcode{false}.
...
\pnum
Class \tcode{system_executor} meets the \oldconcept{Destructible} (\CppXref{destructible}), \oldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), and \newoldconcept{Executor}~(\ref{async.reqmts.executor}) type requirements.

\pnum
To meet the \newoldconcept{Executor} requirements for the \tcode{post} and \tcode{defer} member functions, the system executor may create \tcode{thread} objects to run the submitted function objects. These \tcode{thread} objects are collectively referred to as system threads.
...
\begin{itemdescr}
\pnum
\effects Equivalent to \exposid{decay-copy}\tcode{(forward<Func>(f))()} (\CppXref{expos.only.func}).
\end{itemdescr}
...
\pnum
The class \tcode{system_context} meets the \newoldconcept{ExecutionContext}~(\ref{async.reqmts.executioncontext}) type requirements.
...
\pnum
The \tcode{executor} class provides a polymorphic wrapper for types that meet the \newoldconcept{Executor} requirements~(\ref{async.reqmts.executor}).
...
\pnum
Class \tcode{executor} meets the requirements of \newoldconcept{Executor}~(\ref{async.reqmts.executor}), \oldconcept{DefaultConstructible} (\CppXref{defaultconstructible}), and \oldconcept{CopyAssignable} (\CppXref{copyassignable}).
...
\begin{itemdescr}
\pnum
Let \tcode{e} be the target object of \tcode{*this}. Let \tcode{a1} be the allocator that was specified when the target was set. Let \tcode{fd} be the result of \exposid{decay-copy}\tcode{(f)} (\CppXref{expos.only.func}).
...
\begin{itemdescr}
\pnum
Let \tcode{e} be the target object of \tcode{*this}. Let \tcode{a1} be the allocator that was specified when the target was set. Let \tcode{fd} be the result of \exposid{decay-copy}\tcode{(f)}.
...
\pnum
\begin{note} The function \tcode{dispatch} meets the requirements for an asynchronous operation~(\ref{async.reqmts.async}), except for the requirement that the operation uses \tcode{post} if it completes immediately. \end{note}
...
\pnum
\begin{note} The function \tcode{post} meets the requirements for an asynchronous operation~(\ref{async.reqmts.async}). \end{note}
...
\pnum
\begin{note} The function \tcode{defer} meets the requirements for an asynchronous operation~(\ref{async.reqmts.async}), except for the requirement that the operation uses \tcode{post} if it completes immediately. \end{note}
...
\pnum
The class template \tcode{strand} is a wrapper around an object of type \tcode{Executor} meeting the \newoldconcept{Executor} requirements~(\ref{async.reqmts.executor}).
...
\pnum
\tcode{strand<Executor>} meets the \newoldconcept{Executor}~(\ref{async.reqmts.executor}) requirements.
...
\pnum
\expects \tcode{Executor} is \tcode{CopyAssignable} (\CppXref{copyassignable}).
...
\pnum
\expects \tcode{Executor} is \tcode{MoveAssignable} (\CppXref{moveassignable}).
...
\pnum
The class template \tcode{use_future_t} defines a set of types that, when passed as a completion token~(\ref{async.reqmts.async.token}) to an asynchronous operation's initiating function, cause the result of the asynchronous operation to be delivered via a \tcode{future}~(\CppXref{futures.uniquefuture}).
...
namespace std::experimental::net::inline @\namespacever@ {
  ...
  public:
    ...
    constexpr use_future_t() noexcept(@is_nothrow_default_constructible_v<allocator_type>@);
...
\pnum
\remarks The return type \tcode{T} meets the \oldconcept{Destructible} (\CppXref{destructible}) and \oldconcept{MoveConstructible} (\CppXref{moveconstructible}) requirements.
...
\item
 the nested type \tcode{return_type} is \tcode{future<invoke_result_t<FD\&, Args...>>}; and
\item
