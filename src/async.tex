...
\pnum
No comparison operator, copy operation, move operation, swap operation, or member functions \tcode{context}, \tcode{on_work_started}, and \tcode{on_work_finished} on these types \changed{shall exit}{exits} via an exception.

\pnum
 The executor copy constructor, comparison operators, and other member functions defined in these requirements \changed{shall}{do} not introduce data races as a result of concurrent calls to those functions from different threads.
...
and \tcode{is_convertible_v<OtherExecutor, Executor>} is \tcode{true}.
}
