...
\pnum
[Example: Given an asynchronous operation with Completion signature \tcode{void(R1 r1, R2 r2)}, an initiating function meeting these requirements may be implemented as follows:
...
\begin{itemdescr}
\pnum
Type: \tcode{completion_handler_type\&} if \tcode{CompletionToken} and \tcode{completion_handler_type} are the same type; otherwise, \tcode{completion_handler_type}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
Type: If \tcode{T} has a nested type \tcode{allocator_type}, \tcode{typename T::allocator_type}. Otherwise \tcode{ProtoAllocator}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
Type: If \tcode{T} has a nested type \tcode{executor_type}, \tcode{typename T::executor_type}. Otherwise \tcode{Executor}.
\end{itemdescr}
...
\pnum
Synchronization: The completion of each system thread synchronizes with (C++Std [intro.multithread]) the corresponding successful \tcode{join()} return.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
Completion signature: \tcode{void()}.
