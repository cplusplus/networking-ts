...
\begin{itemdescr}
\pnum
\returns INVOKE\tcode{(get(), forward<Args>(args)...)} (\CppXref{func.require}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to DECAY_COPY\tcode{(forward<Func>(f))()} (\CppXref{thread.decaycopy}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
Let \tcode{e} be the target object of \tcode{*this}. Let \tcode{a1} be the allocator that was specified when the target was set. Let \tcode{fd} be the result of \tcode{DECAY_COPY(f)} (\CppXref{thread.decaycopy}).
