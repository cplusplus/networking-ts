...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{SyncReadStream} requirements if it \changed{satisfies}{meets} the requirements listed in Table~\ref{tab:buffer.stream.reqmts.syncreadstream.requirements}.
...
\tcode{mb} denotes a \removed{(possibly const)} value of a \added{(possibly const)} type \changed{satisfying}{meeting} the \tmpnewoldconcept{MutableBufferSequence}~(\ref{buffer.reqmts.mutablebuffersequence}) requirements,
and \tcode{ec} denotes an object of type \tcode{error_code}.

\indexlibrary{\idxcode{read_some}}%
\begin{libreqtab3}
{\tmpnewoldconcept{SyncReadStream} requirements}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{AsyncReadStream} requirements if it \changed{satisfies}{meets} the requirements listed below.
...
\begin{libreqtab3}
{\tmpnewoldconcept{AsyncReadStream} requirements}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{SyncWriteStream} requirements if it \changed{satisfies}{meets} the requirements listed below.

\pnum
In the table below,
\tcode{a} denotes a value of type \tcode{X},
\tcode{cb} denotes a \removed{(possibly const)} value of a \added{(possibly const)} type \changed{satisfying}{meeting} the \tmpnewoldconcept{ConstBufferSequence}~(\ref{buffer.reqmts.constbuffersequence}) requirements,
and \tcode{ec} denotes an object of type \tcode{error_code}.

\indexlibrary{\idxcode{write_some}}%
\begin{libreqtab3}
{\tmpnewoldconcept{SyncWriteStream} requirements}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{AsyncWriteStream} requirements if it \changed{satisfies}{meets} the requirements listed below.
...
\begin{libreqtab3}
{\tmpnewoldconcept{AsyncWriteStream} requirements}
...
\pnum
A type \tcode{X} meets the \tmpdefnnewoldconcept{CompletionCondition} requirements if it \changed{satisfies}{meets} the requirements of \tmpoldconcept{Destructible} (\CppXref{destructible}) and \tmpoldconcept{CopyConstructible} (\CppXref{copyconstructible}), as well as the additional requirements listed below.
...
and \tcode{n} denotes a \removed{(possibly const)} value of type \added{(possibly const)} \tcode{size_t}.

\begin{libreqtab3}
{\tmpnewoldconcept{CompletionCondition} requirements}
...
\pnum
The class \tcode{transfer_all} \changed{satisfies}{meets} the \tmpnewoldconcept{CompletionCondition}~(\ref{buffer.stream.reqmts.completioncondition}) requirements.
...
\pnum
The class \tcode{transfer_at_least} \changed{satisfies}{meets} the \tmpnewoldconcept{CompletionCondition}~(\ref{buffer.stream.reqmts.completioncondition}) requirements.
...
\pnum
The class \tcode{transfer_exactly} \changed{satisfies}{meets} the \tmpnewoldconcept{CompletionCondition}~(\ref{buffer.stream.reqmts.completioncondition}) requirements.
...
\pnum
\changed{\remarks This function shall not participate in overload resolution unless}{\constraints{}}
\tcode{is_const_buffer_seq\-uence\added{_v}<ConstBufferSequence>\removed{::value}} is \tcode{true}.
\end{itemdescr}
...
\begin{itemize}
\item
the readable bytes of \tcode{b} contains the delimiter \tcode{delim}; or
\item
\tcode{b.size() == b.max_size()}; or
\item
...
\pnum
On exit, if the readable bytes of \tcode{b} contains the delimiter, \tcode{ec} is set such that \tcode{!ec} is \tcode{true}. Otherwise, if \tcode{b.size() == b.max_size()}, \tcode{ec} is set such that \tcode{ec == stream_errc::not_found}. If \tcode{b.size() < b.max_size()}, \tcode{ec} contains the \tcode{error_code} from the most recent \tcode{read_some} call.

\pnum
\returns The number of bytes in the readable bytes of \tcode{b} up to and including the delimiter, if present. \begin{note} On completion, the buffer can contain additional bytes following the delimiter. \end{note} Otherwise returns \tcode{0}.
\end{itemdescr}
